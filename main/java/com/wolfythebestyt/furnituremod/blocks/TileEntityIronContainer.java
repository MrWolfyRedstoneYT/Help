...
public class TileEntityIronContainer extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return new ContainerIronContainerBlock(playerInventory, this, playerIn);
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerIronContainerBlock)
                {
                    if (((ContainerIronContainerBlock)entityplayer.openContainer).getChestInventory() == this)
                    {
